...
export default function TemperatureLogs() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Fetch history
                const data = await api.sensors.getHistory('temperature', { limit: 50 });
    ...
    const getStatusBadge = (value) => {
        if (value > 40) return { label: 'Critical', class: 'bg-red-600 text-white' };
        if (value > 35) return { label: 'High', class: 'bg-orange-600 text-white' };
        if (value > 30) return { label: 'Medium', class: 'bg-yellow-600 text-white' };
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
            {/* Header */}
            <div>
                <h1 className="text-3xl font-bold text-white mb-1">Temperature Sensor Logs</h1>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                        <div className="space-y-1">
                            <div className="flex items-baseline gap-2">
                                ...
                                <span className="text-3xl text-orange-400/70">°C</span>
                            </div>
                        ...
                        <div className="relative w-64 h-32">
                            <svg viewBox="0 0 200 100" className="w-full h-full overflow-visible">
                                ...
                                <path
                                    ...
                                    strokeDashoffset={251.2 * (1 - (Math.min(Math.max(stats?.latest?.value || 0, 0), 60) / 60))}
                        ...
                        <table className="min-w-full text-sm text-gray-100">
                            <thead className="border-b border-gray-600 text-xs uppercase text-gray-400">
                                <tr>
                                    ...
                                    <th className="py-3 text-left font-semibold">Temperature</th>
